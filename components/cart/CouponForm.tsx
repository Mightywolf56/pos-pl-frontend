...
export default function CouponForm() {
  ...
  const handleSubmitCoupon = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
  ...
  return (
    <>
      ...
      <form className="flex" onSubmit={handleSubmitCoupon}>
        <input
          type="text"
        ...
        <input
          type="submit"
          className="p-3 bg-green-300 font-bold hover:cursor-pointer"
          value="Canjear"
        />
      </form>
      {coupon.message ? (
        <p className='py-4 text-center text-md font-bold'>{coupon.message}</p>
      ) : null}
    </>
  );
}
